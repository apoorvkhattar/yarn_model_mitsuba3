...
public:
    ...
    BSplineCurve(const Properties &props) : Base(props) {
        // Custom Yarn Id
        m_yarn_number = props.get<ScalarFloat>("yarn_number", 1.f);
        ...
        std::vector<InputPoint3f> vertices_normal;
        ...
        while (ptr < eof) {
            ...
            // Vertex position
            InputPoint3f p; InputPoint3f v;
            for (ScalarSize i = 0; i < 6; ++i) {
                const char *orig = cur;
                if(i < 3){
                    ...
                    v[i%3] = string::strtof<InputFloat>(cur, (char **) &cur);
                    parse_error |= cur == orig;
                }
        ...
        std::unique_ptr<InputFloat[]> positions_normals =
            std::make_unique<InputFloat[]>(m_control_point_count * 3);
        ...
        for (ScalarIndex i = 0; i < vertices.size(); i++) {
            ...
            InputFloat *normal_ptr = positions_normals.get() + i * 3;
            dr::store(normal_ptr, vertices_normal[i]);
        }
        m_vertex = dr::load<FloatStorage>(positions.get(), m_control_point_count * 3);

        m_vertex_normals = dr::load<FloatStorage>(positions_normals.get(), m_control_point_count * 3);
    ...
    SurfaceInteraction3f eval_parameterization(const Point2f &uv,
        ...
        std::tie(v_rot, v_rad) = local_frame(c, u_local, dc_du_normalized, segment_id, active);
        ...
        // Mask hit_mask = checkHit(si, ray, c, active);
        // si.t = dr::select(hit_mask && active, si.t, dr::Infinity<Float>);
        
        si.finalize_surface_interaction(pi, ray, ray_flags, active);

        // si.prim_index = m_yarn_number;
        // pi.prim_index = m_yarn_number;
    ...
    SurfaceInteraction3f compute_surface_interaction(const Ray3f &ray,
        ...
        Vector3f v_rot, v_rad;
        std::tie(v_rot, v_rad) = local_frame(c, u_local, dc_du_normalized, idx, active);
        ...
        if constexpr (IsDiff) {
            ...
            v *= dr::InvTwoPi<Float>;
            // Float v = dr::acos(dr::dot(v_rot, rad_vec_normalized));
            // v *= dr::InvPi<Float>;
        ...
        // si.time = dr::select(active, m_yarn_number, si.time);
        ...
        si.dn_du = Vector3f(m_yarn_number);
...
private:
    ...
    std::tuple<Vector3f, Vector3f>
    local_frame(const Point3f c, const Float u, const Vector3f &dc_du_normalized, const Index &idx, Mask active) const {
        // Define consistent local frame using the curvature
        // (1) Consistently define a rotation axis (`v_rot`) that lies in the hemisphere defined by `guide`
        // (2) Rotate `dc_du` by 90 degrees on `v_rot` to obtain `v_rad`
        Vector3f n0 = dr::gather<Vector3f>(m_vertex_normals, idx + 0, active),
                n1 = dr::gather<Vector3f>(m_vertex_normals, idx + 1, active),
                n2 = dr::gather<Vector3f>(m_vertex_normals, idx + 2, active),
                n3 = dr::gather<Vector3f>(m_vertex_normals, idx + 3, active);
        
        // Point3f v0 = dr::gather<Point3f>(m_vertex, idx + 0, active),
        //         v1 = dr::gather<Point3f>(m_vertex, idx + 1, active);
        // Float d0 = dr::norm(c - v0), d1 = dr::norm(c - v1);
        // Vector3f v_rot = dr::normalize(d1/(d0+d1) * n0 + d0/(d0+d1) * n1);

        Float u2 = dr::sqr(u), u3 = u2 * u;
        Float multiplier = 1.f / 6.f;

        Vector3f v_rot = (-u3 + 3.f * u2 - 3.f * u + 1.f) * n0 +
                    (3.f * u3 - 6.f * u2 + 4.f) * n1 +
                    (-3.f * u3 + 3.f * u2 + 3.f * u + 1.f) * n2 +
                    u3 * n3;
        v_rot *= multiplier;
        v_rot = dr::normalize(v_rot);

        Vector3f v_rad = dr::normalize(dr::cross(v_rot, dc_du_normalized));

        return { v_rot, v_rad };
    }

    Mask checkHit(const SurfaceInteraction3f si, const Ray3f ray, Point3f c, Mask active) const {
        Mask hit_mask = false;
        // Vector3f ray_projection;
        // ray_projection = dr::normalize(ray.d - dr::dot(ray.d, si.sh_frame.s) * si.sh_frame.s);

        // for(int i = 0; i < 4; i++){
        //     Float fi = Float(i);

        //     Point3f center;
        //     center.x() = dr::cos(dr::TwoPi<Float> * fi / 4); 
        //     center.y() = dr::sin(dr::TwoPi<Float> * fi / 4); 
        //     center.z() = si.p.z();
            
        //     Transform4f correction = Transform4f::rotate(Vector3f(0,0,1), 1/3 * si.uv.x() * 180 * dr::InvPi<Float>);
        //     center = correction.transform_affine(center);

        //     Vector3f dist = (center - si.p) - dr::dot(center - si.p, ray_projection) * ray_projection;

        //     hit_mask = dr::norm(dist) < 1 || hit_mask;
        ...
        for(int i = 0; i < 1; i++){
            ...
            Vector3f dist = (c - si.p) - dr::dot(c - si.p, ray_projection) * ray_projection;
