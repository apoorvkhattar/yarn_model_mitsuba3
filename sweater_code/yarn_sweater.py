...
rad = 2. # Yarn Radius
sigma = 10.5 # Sigma For Attenuation of Specular Y
sigma_d = 1.0 # Sigma For Attenuation of Specular Y # Basket
...
ply_shadow_thr = 0.0 # Adjust AO to make it between n to 1 instead of 0 to 1
...
def intersectWithScene(scene, rays, active, plyCS, scale=scale_V):
    ...
    colors = mi.Color3f(0)
    colors = dr.select(dr.eq(mi.Int(yarn_number) % 3, 0), mi.Color3f((0.01, 0.00, 0.09)), colors)
    colors = dr.select(dr.eq(mi.Int(yarn_number) % 3, 1), mi.Color3f((0.01, 0.09, 0.00)), colors)
    colors = dr.select(dr.eq(mi.Int(yarn_number) % 3, 2), mi.Color3f((0.09, 0.00, 0.01)), colors)
...
def ray_test(scene2, rays2, active2, scale2):
    ...
    si2.uv.x = uv.y 
    si2.uv.y = uv.x * scale2 # V in [0,1] so scale it to make the ply twist more visible
...
#         orig_si = mi.SurfaceInteraction3f(si)
        ...
        # emitter_ray = si.spawn_ray(ds.d)
        occluded = ray_test(scene, emitter_ray, si.is_valid(), scale2=scale_V)
        ...
        # Specular Component
        # attn_fac = dr.select(~prev_mask, 1, dr.power(dr.abs(dr.exp(-sigma * dr.norm(si.p-prev_si.p))), ply_count))
        ...
        # Body Component
        # attn_fac_d = dr.select(~prev_mask, 1, dr.power(dr.abs(dr.exp(-sigma_d * dr.norm(si.p-prev_si.p))), ply_count))
...
pl = np.loadtxt('./output.txt') * 10
count_list = np.loadtxt('./point.txt').astype(np.int32)
